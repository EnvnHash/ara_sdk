...
#include "SNAudioWave.h"

namespace tav
{
SNAudioWave::SNAudioWave(sceneData* _scd,
		...
		SceneNode(_scd, _sceneArgs)
{
	...
	for (auto i = 0; i < scd->nrChannels; i++)
	{
		...
		lines[i]->setColor(1.f, 1.f, 1.f, 1.f);
	}
}

//----------------------------------------------------

SNAudioWave::~SNAudioWave()
{
}

//----------------------------------------------------

void SNAudioWave::draw(double time, double dt, camPar* cp, Shaders* _shader,
		TFO* _tfo)
{
	if (_tfo)
	{
...
void SNAudioWave::update(double time, double dt)
{
}

//----------------------------------------------------

void SNAudioWave::updateWave()
{
	for (auto chanNr = 0; chanNr < scd->nrChannels; chanNr++)
	{
		...
		for (int n = 0; n < lines[chanNr]->nrSegments; n++)
		{
			...
			{
				float val = pa->getSampDataAtPos(scd->chanMap[chanNr],
