...
namespace tav
{
    SNTFluid::SNTFluid(sceneData* _scd, std::map<std::string, float>* _sceneArgs) :
    ...
    void SNTFluid::draw(double time, double dt, camPar* cp, Shaders* _shader, TFO* _tfo)
    {
        if (_tfo)
        {
    ...
    void SNTFluid::update(double time, double dt)
    {
        glm::vec2 pos, vel, c;
        glm::vec4 col;
