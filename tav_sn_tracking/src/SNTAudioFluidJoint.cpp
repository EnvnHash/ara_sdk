...
#include "SNTAudioFluidJoint.h"

namespace tav
{
    SNTAudioFluidJoint::SNTAudioFluidJoint(sceneData* _scd, std::map<std::string, float>* _sceneArgs) :
    ...
    SNTAudioFluidJoint::~SNTAudioFluidJoint()
    {
        fluidSim->cleanUp();
        delete fluidSim;
        delete quadAr;
    ...
    void SNTAudioFluidJoint::draw(double time, double dt, camPar* cp, Shaders* _shader, TFO* _tfo)
    {
        if (_tfo)
        {
...
//        fluidSim->draw();
    ...
    void SNTAudioFluidJoint::update(double time, double dt)
    {
        glm::vec2 pos, vel, c;
        glm::vec4 col;
