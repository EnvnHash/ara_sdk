...
namespace tav
{
    SNTAugRoom::SNTAugRoom(sceneData* _scd, std::map<std::string, float>* _sceneArgs) :
...
//        kpCalib.beamerAspectRatio = static_cast<float>(scd->screenWidth) / static_cast<float>(scd->screenHeight);
        ...
        kalmanRotZ = new CvKalmanFilter(6, 3, 0.005f);
        kalmanTrans = new CvKalmanFilter(6, 3, 0.001f);

        medTrans = new Median<glm::vec3>(2.f);
        medTransPost = new Median<glm::vec3>(7.f);

        medRot = new Median<glm::vec3>(2.f);
        medRotPost = new Median<glm::vec3>(7.f);

        medRotXY = new Median<glm::vec3>(2.f);
        medRotXYPost = new Median<glm::vec3>(7.f);
...
//        kpCalib.beamerFovY = kpCalib.beamerFovX / kpCalib.beamerAspectRatio;
        ...
        if(actDrawMode == CHESS_TO_CAM )  kpCalib.invertMatr = false;
...
//                                                              "shaders/basic_tex_alpha.frag");
    	...
    	// preset SceneNode modelMatrix
//    	_medTrans->clear();
//    	_medScale->update(glm::vec3(3.f, 3.f, 1.f));
//    	scale();
    	_medTrans->update(glm::vec3(0.f, -3.f, 0.f));
    	translate();
    	rebuildModelMat();
    }
    ...
    void SNTAugRoom::initKinDependent()
    {
    ...
    void SNTAugRoom::update(double time, double dt)
    {
        ...
        if (isInited)
        {
            ...
            if (gotNewColFrame)
            {
                ...
                if ( frameNr > 20 && (actDrawMode == CHESS_TO_CAM || actDrawMode == SPACE) )
                {
                    ...
                    chessRotMatr = getRotRealToGL(kpCalib, found);

                    if (found && actDrawMode == CHESS_TO_CAM)
                    {
    ...
    void SNTAugRoom::draw(double time, double dt, camPar* cp, Shaders* _shader, TFO* _tfo)
    {
        if (_tfo) {
    ...
    void SNTAugRoom::drawKinOverlay()
    {
        shdr->begin();
    ...
    void SNTAugRoom::drawChessToCam(double time, double dt)
    {
        ...
        blendShader->begin();
        blendShader->setUniform1f("alpha", 1.f);
        blendShader->setUniform1i("tex", 0);
...
//        xLine->draw(GL_LINES);
    ...
    void SNTAugRoom::drawSpace(double time, double dt, camPar* cp)
    {
        ...
        glm::mat4 mod =  chessRotMatr * _modelMat;
    ...
    void SNTAugRoom::getRealWorldKinBeamerOffs(kpCalibData& kp)
    {
    ...
    glm::mat4 SNTAugRoom::getRotRealToCamToGL(kpCalibData& kp)
    {
        glm::mat4 out;
        std::vector<glm::vec3> realWorldCoord;
        glm::vec4 rotated;
        ...
        kp.chessRotXY = calcChessRotXY(realWorldCoord, kp, true);
        // get the translation
        kp.chessTrans = calcChessTrans(realWorldCoord, kp, true, false);
        // get rotation around the z-axis
        kp.chessRotZ = calcChessRotZ(realWorldCoord, kp, true);
    ...
    glm::mat4 SNTAugRoom::getRotRealToGL(kpCalibData& kp, bool found)
    {
        glm::mat4 out;
        glm::vec3 midPoint;
        std::vector<glm::vec3> realWorldCoord;
        ...
        // get rotation around y and z axis
        kp.chessRotXY = calcChessRotXY(realWorldCoord, kp, found);
        // get the translation
        kp.chessTrans = calcChessTrans(realWorldCoord, kp, found);
        // get rotation around the z-axis
        kp.chessRotZ = calcChessRotZ(realWorldCoord, kp, found);


        // combine the three transformations
 //       out = kp.chessTrans * kp.chessRotXY;
        out = kp.chessRotZ * kp.chessRotXY * kp.chessTrans;
//        out = kp.chessTrans * kp.chessRotXY * kp.chessRotZ;
    ...
    void SNTAugRoom::getRealWorldCoord(glm::vec3& inCoord, glm::vec2& _kinRealWorldXYScale)
    {
    ...
    void SNTAugRoom::getRealWorldCoordUnwarp(glm::vec3& inCoord, glm::vec2& _kinRealWorldXYScale,
...
//            cout << dp[0] << endl;
    ...
    glm::vec3 SNTAugRoom::getChessMidPoint(std::vector<glm::vec3>& _realWorldCoord, kpCalibData& kp)
    {
    ...
    glm::mat4 SNTAugRoom::calcChessRotXY(std::vector<glm::vec3>& _realWorldCoord, kpCalibData& kp, bool found)
    {
        ...
        if ((int) normals.size() > 0 || !found)
        {
        	if (found) outNorm /= (float)normals.size();
            ...
            if (found)
            {
                medRotXY->update(outNorm);
            	kalmanRot->update(medRotXY->get().x, medRotXY->get().y, medRotXY->get().z);
            }

            cv::Mat smoothed = kalmanRot->getPrediction();

            medRotXYPost->update(
            		glm::vec3(smoothed.at<float>(0),
            				  smoothed.at<float>(1),
            				  smoothed.at<float>(2)));

            kp.chessNormal = medRotXYPost->get();

            if (!kp.invertMatr) {
                outMat = glm::mat4(RotationBetweenVectors(kp.chessNormal, glm::vec3(0.f, 0.f, -1.f)));
            } else if (actDrawMode == SPACE) {
            	//kp.chessNormal.x *= -1.f;
                outMat = glm::mat4(RotationBetweenVectors(kp.chessNormal, glm::vec3(0.f, 0.f, -1.f)));
            } else {
    ...
    glm::mat4 SNTAugRoom::calcChessRotZ(std::vector<glm::vec3>& _realWorldCoord, kpCalibData& kp, bool found)
    {
        glm::mat4 outMat = glm::mat4(1.f);
        ...
        if (found)
        {
        	...
        	outLeftVec = glm::vec4(0.f, 0.f, 0.f, 0.f);
        	for (std::vector<glm::vec4>::iterator it = leftVecs.begin(); it != leftVecs.end(); ++it)
        		if (!std::isnan((*it).x) && !std::isnan((*it).y) && !std::isnan((*it).z))
        			outLeftVec += (*it);
        }

        if ( static_cast<int>(leftVecs.size()) > 0 || !found )
        {
            if(found) outLeftVec /= (float)leftVecs.size();

            // apply kalman filter
            kalmanRotZ->predict();

            if(found){
            	medRot->update( glm::vec3(outLeftVec) );
            	kalmanRotZ->update(medRot->get().x, medRot->get().y, medRot->get().z);
            }
    ...
    glm::mat4 SNTAugRoom::calcChessTrans(std::vector<glm::vec3>& _realWorldCoord, kpCalibData& kp,
    		bool found, bool normalize)
    {
        glm::mat4 out;
           ...
           && kp.chessRealMid.z != 0.f) || !found)
        {
            ...
            if ((std::isfinite(kp.chessRealMid.x)
                ...
                && std::isfinite(kp.chessRealMid.z)) || !found )
            {
                // pre filter
                if (found) medTrans->update(kp.chessRealMid);

                if (!found) std::cout << "predict only " << std::endl;

                kalmanTrans->predict();

                if (found)
                	kalmanTrans->update(medTrans->get().x,
                			medTrans->get().y,
							std::min(std::max(medTrans->get().z, 0.0001f), 1000.f));

                cv::Mat smoothed = kalmanTrans->getPrediction();
                glm::vec3 smoothedVec3 = glm::vec3(smoothed.at<float>(0),
                        							smoothed.at<float>(1),
													smoothed.at<float>(2));
                ...
                medTransPost->update(smoothedVec3);
                out = glm::translate(glm::mat4(1.f), medTransPost->get());
            }
        }
    ...
    glm::mat4 SNTAugRoom::rtMatToGlm(int imgW, int imgH, cv::Mat& rVec, cv::Mat& tVec, bool inv)
    {
    ...
    glm::mat4 SNTAugRoom::getBeamerFromCv(cv::Mat& camMatr, float calibWidth, float calibHeight, float znear,
                                           float zfar, bool yUp)
    {
        glm::mat4 outM = glm::mat4(1.f);
    ...
    cv::Mat SNTAugRoom::getPerspTrans(std::vector<cv::Point2f>& pointbuf)
    {
        cv::Mat out;
    ...
    void SNTAugRoom::loadCamToBeamerCalib(cv::FileStorage& fs, kpCalibData& kp)
    {
    ...
    void SNTAugRoom::saveCamToBeamerCalib(std::string _filename, kpCalibData& kp)
    {
    ...
    cv::Mat SNTAugRoom::glmToCvMat(glm::mat4& mat)
    {
    ...
    glm::mat4 SNTAugRoom::cvMatToGlm(cv::Mat& _mat)
    {
    ...
    glm::mat4 SNTAugRoom::cvMat33ToGlm(cv::Mat& _mat)
    {
        glm::mat4 out = glm::mat4(1.f);
        for (short j=0;j<3;j++)
            for (short i=0;i<3;i++)
    ...
    void SNTAugRoom::onKey(int key, int scancode, int action, int mods)
    {
    ...
    void SNTAugRoom::onCursor(GLFWwindow* window, double xpos, double ypos)
    {
    ...
    void SNTAugRoom::onMouseButton(GLFWwindow* window, int button, int action, int mods)
    {
    }

    //------------------------------------------------------------------------

    SNTAugRoom::~SNTAugRoom()
    {
        delete colShader;
        delete blendShader;
        delete quad;
