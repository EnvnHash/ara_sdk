...
SNMural::SNMural(sceneData* _scd, std::map<std::string, float>* _sceneArgs) :
		SceneNode(_scd, _sceneArgs),
		actDrawMode(DRAW),  // RAW_DEPTH, DEPTH_THRESH, DRAW
		depthThresh(2200.f),
		fastBlurAlpha(0.89f),
		noActivityThresh(382.f),
		recordThresh(83.f),
		secondThres(0.93f),
		resetSnapShotTime(3.0),
		trig(0.f),
		clear(0.f)
{
...
void SNMural::initDepthThresh()
{
	...
	depthThres = shCol->addCheckShaderText("SNMural_thres", stdVert.c_str(), frag.c_str());

}

//---------------------------------------------------------------

void SNMural::initDepthThresh2()
{
	std::string shdr_Header = "#version 410 core\n#pragma optimize(on)\n";
	std::string stdVert = STRINGIFY(layout( location = 0 ) in vec4 position;
								 layout( location = 1 ) in vec4 normal;
	...
	depthThres2 = shCol->addCheckShaderText("SNMural_thres2", stdVert.c_str(), frag.c_str());

}

//---------------------------------------------------------------

void SNMural::initSilShdr()
{
	std::string shdr_Header = "#version 410 core\n#pragma optimize(on)\n";
	std::string stdVert = STRINGIFY(layout( location = 0 ) in vec4 position;
								 layout( location = 1 ) in vec4 normal;
	...
	silShdr = shCol->addCheckShaderText("SNMural_sill", stdVert.c_str(), frag.c_str());

}

//---------------------------------------------------------------

void SNMural::initSnapShotShdr()
{
	std::string shdr_Header = "#version 410 core\n#pragma optimize(on)\n";
	std::string stdVert = STRINGIFY(layout( location = 0 ) in vec4 position;
								 layout( location = 1 ) in vec4 normal;
	...
	snapShotShdr = shCol->addCheckShaderText("SNMural_snap", stdVert.c_str(), frag.c_str());
}

//----------------------------------------------------

void SNMural::initDiffShdr()
{
	...
	frag = "// SNMural diff frag\n"+shdr_Header+frag;

	diffShader = shCol->addCheckShaderText("SNMural_diff", vert.c_str(), frag.c_str());
}

//---------------------------------------------------------------

void SNMural::draw(double time, double dt, camPar* cp, Shaders* _shader, TFO* _tfo)
{
		switch(actDrawMode)
		{
			case RAW_DEPTH : // 1
...
void SNMural::update(double time, double dt)
{
	if (kin && kin->isReady())
	{
		if (!inited)
...
void SNMural::onKey(int key, int scancode, int action, int mods)
{
