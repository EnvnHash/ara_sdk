...
#include "SNSimpTexture.h"

namespace tav
{
SNSimpTexture::SNSimpTexture(sceneData* _scd, std::map<std::string, float>* _sceneArgs) :
...
SNSimpTexture::~SNSimpTexture()
{
}

//----------------------------------------------------

void SNSimpTexture::draw(double time, double dt, camPar* cp, Shaders* _shader, TFO* _tfo)
{
	if (_tfo) {
	...
	glm::mat4 m = glm::translate( glm::vec3(0.68f, 0.68f, 0.f) )
...
//			* glm::scale(glm::vec3(cp->actFboSize.y / cp->actFboSize.x, img0Propo, 1.f));
	...
	stdTexAlpha->setUniformMatrix4fv("m_pvm", &m[0][0]);
...
void SNSimpTexture::update(double time, double dt)
{
}
}
