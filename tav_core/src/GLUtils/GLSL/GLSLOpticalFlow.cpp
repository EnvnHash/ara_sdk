...
namespace tav
{
GLSLOpticalFlow::GLSLOpticalFlow(ShaderCollector* _shCol, int _width,
		int _height) :
...
void GLSLOpticalFlow::initShader(ShaderCollector* _shCol)
{
	...
	std::string vert =
			STRINGIFY(
					layout( location = 0 ) in vec4 position; layout( location = 1 ) in vec4 normal; layout( location = 2 ) in vec2 texCoord; layout( location = 3 ) in vec4 color; uniform mat4 m_pvm;
	...
	std::string frag =
			STRINGIFY(
					uniform sampler2D tex0; uniform sampler2D tex1; uniform vec2 scale; uniform vec2 offset; uniform float amp; uniform float lambda; uniform float median;
					...
					vec4 getGrayScale(vec4 col) { float gray = dot(vec3(col.x, col.y, col.z), vec3(0.3, 0.59, 0.11)); return vec4(gray,gray,gray,1.0); }

					vec4 texture2DRectGray(sampler2D tex, vec2 coord) { return getGrayScale(texture(tex, coord)); }

					void main() { vec4 a = texture2DRectGray(tex0, tex_coord); vec4 b = texture2DRectGray(tex1, tex_coord); vec2 x1 = vec2(offset.x, 0.0); vec2 y1 = vec2(0.0, offset.y);
					...
					color = vec4(pow(abs(vx.r) * 6.0, 1.7) * sign(vx.r) * amp, pow(abs(vy.r) * 6.0, 1.7) * sign(vy.r) * amp, 0.0, 1.0); });

	frag = "// Optical Flow Shader pos tex shader\n" + shdr_Header + frag;

	flowShader = _shCol->addCheckShaderText("StandardOpticalFlow", vert.c_str(),
			frag.c_str());
}
...
void GLSLOpticalFlow::setMedian(float _median)
{
	median = _median;
}
...
void GLSLOpticalFlow::setBright(float _val)
{
	bright = _val;
}
...
void GLSLOpticalFlow::setPVM(GLfloat* _pvm_ptr)
{
	pvm_ptr = _pvm_ptr;
}
...
GLuint GLSLOpticalFlow::getResTexId()
{
