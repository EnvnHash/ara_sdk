...
SkyBoxBlend::SkyBoxBlend(std::string textureFile, unsigned _nrCams)
{
...
SkyBoxBlend::SkyBoxBlend(const char* textureFile, unsigned _nrCams)
{
...
void SkyBoxBlend::init(const char* textureFile, unsigned _nrCams)
{
	vShader =
			STRINGIFY(
	...
	gShader =
			STRINGIFY(
					in vec4 pos[]; out vec3 tex_coord; out vec3 tex_coord_2; out vec3 tex_coord_3; uniform mat4 tc_rot; uniform mat4 tc_rot2; uniform mat4 tc_rot3; void main(void) { gl_ViewportIndex = gl_InvocationID; for (int i=0;i<gl_in.length();i++) { tex_coord = normalize( (tc_rot * pos[i]).xyz ); tex_coord_2 = normalize( (tc_rot2 * pos[i]).xyz ); tex_coord_3 = normalize( (tc_rot3 * pos[i]).xyz ); gl_Position = m_pvm[gl_InvocationID] * pos[i];
	...
	fShader =
			STRINGIFY(
					uniform vec2 noiseScale; uniform float width; uniform float height; uniform float time; uniform float time2; in vec3 tex_coord; in vec3 tex_coord_2; in vec3 tex_coord_3; layout (location = 0) out vec4 color; uniform samplerCube tex;
					...
					float noiseC = 0.0; float noiseC2 = 0.0; const vec3 LumCoeff = vec3(0.2125, 0.7154, 0.0721); const vec3 AvgLumin = vec3(0.5, 0.5, 0.5);

					void main(void) { vec2 posCo = vec2(gl_FragCoord) / vec2(width, height) * noiseScale;

					noiseC = ( cnoise( vec3( posCo *32.0 , time) ) + 1.0 ) * 0.5; noiseC = min( max( pow(noiseC, 2.0) * 1.5, 0.0), 1.0);

					noiseC2 = ( cnoise( vec3( posCo *16.0, time2) ) + 1.0 ) * 0.5; noiseC2 = min( max( pow(noiseC2, 2.0) * 1.5, 0.0), 1.0);
					//color = vec4(noiseC, noiseC, noiseC, 1.0);

					vec4 tex0 = texture(tex, tex_coord); vec4 tex2 = texture(tex, tex_coord_2); vec4 tex3 = texture(tex, tex_coord_3);

					vec4 upperLayer = tex0 * noiseC + tex2 * (1.f - noiseC); upperLayer = upperLayer * noiseC2 + tex3 * (1.f - noiseC2);

					vec3 intensity = vec3(dot(upperLayer.rgb, LumCoeff));

					// could substitute a uniform for this 1. and have variable saturation
					vec3 satColor = mix(intensity, upperLayer.rgb, 1.); vec3 conColor = mix(AvgLumin, satColor, 1.2);// constrast
...
void SkyBoxBlend::draw(double time, camPar* cp, Shaders* _shader, TFO* _tfo)
{
	...
	glm::mat4 rotMat2 = glm::rotate(rotMat, float(M_PI),
			glm::vec3(std::sin(time * 0.8f) * 0.4f, 0.8f, 0.2f));
	glm::mat4 rotMat3 = glm::rotate(rotMat, float(M_PI) * 0.3f,
			glm::vec3(std::sin(time * 0.6f) * -0.4f, 0.8f, 0.f));
	...
	sbShader->setUniform1f("time", time);
	sbShader->setUniform1f("time2", time * 2.0f);
	cubeTex->bind(0);
...
void SkyBoxBlend::setMatr(glm::mat4* _inMatr)
{
	modMatr = _inMatr;
}
