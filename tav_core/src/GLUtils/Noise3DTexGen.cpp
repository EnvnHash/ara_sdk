...
// funktioniert auf linux maschine nur, wenn in draw schleife initialisiert...
...
namespace tav
{
Noise3DTexGen::Noise3DTexGen(ShaderCollector* _shCol, bool color, int nrOctaves,
		int _width, int _height, int _depth, float _scaleX, float _scaleY,
		float _scaleZ) :
		shCol(_shCol), width(_width), height(_height), scaleX(_scaleX), scaleY(
				_scaleY), scaleZ(_scaleZ)
{
	depth = _depth;
	nrLoops = _depth;
	...
	fbo = new FBO(_shCol, width, height, depth, GL_RGBA8, GL_TEXTURE_3D, false, 1, 1, 1, GL_REPEAT, false);
	xBlendFboH = new FBO(_shCol, width, height, GL_RGBA8, GL_TEXTURE_2D, false, 1, 1, 1, GL_REPEAT, false);
	xBlendFboV = new FBO(_shCol, width, height, GL_RGBA8, GL_TEXTURE_2D, false,	1, 1, 1, GL_REPEAT, false);

	quad = new Quad(-1.f, -1.f, 2.f, 2.f, glm::vec3(0.f, 0.f, 1.f), 0.f, 0.f, 0.f, 1.f);
	fboQuad = new Quad(0.f, 0.f, 1.f, 1.f, glm::vec3(0.f, 0.f, 1.f), 0.f, 0.f,	0.f, 1.f);
	...
	for (short i = 0; i < nrLoops; i++)
	{
		zPos[0] = float(i) / float(nrLoops - 1);
		...
		xBlendShaderH->setUniform1f("zPos",
				zPos[0] + 0.5f / float(nrLoops - 1));
...
void Noise3DTexGen::initShdr()
{
	...
	std::string frag = STRINGIFY(
					...
					void main() { modTexCoord = vec2(scaleX * texCo.x, scaleY * texCo.y); if(useColor == 1) { vec3 noise = vec3(0.0); for (col=0;col<3;col++) for (i=0;i<nrOctaves;i++) noise[col] += cnoise( vec3( modTexCoord * pow(2.0, float(i)), zPos + float(col) * 0.3 ) );
					//noise /= max(nrOctaves / 2, 1);
					fragColor = vec4( noise, 1.0 ); } else { float noise = 0.0; for (i=0;i<nrOctaves;i++) noise += cnoise( vec3( modTexCoord * pow(2.0, float(i)), zPos ) );
					//noise /= max(nrOctaves / 2, 1);
					fragColor = vec4( noise, noise, noise, 1.0 ); } });
	frag = "// Noise3DTexGen pos tex shader\n" + shdr_Header + frag;

	noiseShdr = shCol->addCheckShaderText("noise3DTexGen", vert.c_str(), frag.c_str());
}

//--------------------------------------------------------------------------------

void Noise3DTexGen::initBlendShdr()
{
	std::string shdr_Header = "#version 410 core\n#pragma optimize(on)\n";
	...
	xBlendShaderH = shCol->addCheckShaderText("noise3DTexGenBlendH", vert.c_str(), frag.c_str());
	...
	xBlendShaderV = shCol->addCheckShaderText("noise3DTexGenBlendV", vert.c_str(), frag.c_str());
}
