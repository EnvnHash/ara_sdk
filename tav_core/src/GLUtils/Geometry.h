...
public:
	...
	void append(Mesh<TVertex>& geo)
	{
		...
		m_indicesTriangles.reserve(
				geo.m_indicesTriangles.size() + m_indicesTriangles.size());
		m_indicesOutline.reserve(
				geo.m_indicesOutline.size() + m_indicesOutline.size());

		glm::uint offset = glm::uint(m_vertices.size());
		...
		{
			m_indicesTriangles.push_back(
					geo.m_indicesTriangles[i] + glm::uvec3(offset));
		}

		for (size_t i = 0; i < geo.m_indicesOutline.size(); i++)
		{
			m_indicesOutline.push_back(
					geo.m_indicesOutline[i] + glm::uvec2(offset));
		}
	}
	...
	{
		return m_indicesTriangles.size() * sizeof(glm::uvec3);
	}

	glm::uint getTriangleIndicesCount() const
	{
		return (glm::uint) m_indicesTriangles.size() * 3;
	}

	size_t getOutlineIndicesSize() const
	{
		return m_indicesOutline.size() * sizeof(glm::uvec2);
	}

	glm::uint getOutlineIndicesCount() const
	{
		return (glm::uint) m_indicesOutline.size() * 2;
	}
	...
	glm::uint getVerticesCount() const
	{
		return (glm::uint) m_vertices.size();
	}
};
...
public:
	static void add(Mesh<TVertex>& geo, const glm::mat4& mat, int w, int h)
	{
		...
		glm::uint vertOffset = (glm::uint) geo.m_vertices.size();
		...
		for (y = 0; y < ydim + 1; y++)
		{
			for (x = 0; x < xdim + 1; x++)
			{
				...
				tav::Vertex vert = tav::Vertex(pos, normal, uv, col);
		...
		for (y = 0; y < ydim; y++)
		{
			for (x = 0; x < xdim; x++)
			{
				...
				geo.m_indicesTriangles.push_back(
						glm::uvec3((x) + (y + 1) * width + vertOffset,
								(x) + (y) * width + vertOffset,
								(x + 1) + (y + 1) * width + vertOffset));
				// lower tris
				geo.m_indicesTriangles.push_back(
						glm::uvec3((x + 1) + (y + 1) * width + vertOffset,
								(x) + (y) * width + vertOffset,
		...
		for (y = 0; y < ydim; y++)
		{
			geo.m_indicesOutline.push_back(
					glm::uvec2((y) * width + vertOffset,
							(y + 1) * width + vertOffset));
		}
		for (y = 0; y < ydim; y++)
		{
			geo.m_indicesOutline.push_back(
					glm::uvec2((y) * width + xdim + vertOffset,
							(y + 1) * width + xdim + vertOffset));
		}
		for (x = 0; x < xdim; x++)
		{
			geo.m_indicesOutline.push_back(
					glm::uvec2((x) + vertOffset, (x + 1) + vertOffset));
		}
		for (x = 0; x < xdim; x++)
		{
			geo.m_indicesOutline.push_back(
					glm::uvec2((x) + ydim * width + vertOffset,
			...
			int d)
	{
		...
		for (int side = 0; side < 6; side++)
		{
			...
			case 0:
				break;
			case 1:
				matrixRot = glm::rotate(glm::mat4(1.f), float(M_PI),
						glm::vec3(0.f, 1.f, 0.f));
				break;
			case 2:
				matrixRot = glm::rotate(glm::mat4(1.f), float(M_PI * 0.5f),
						glm::vec3(0.f, 1.f, 0.f));
				;
				break;
			case 3:
				matrixRot = glm::rotate(glm::mat4(1.f), float(M_PI * 1.5f),
						glm::vec3(0.f, 1.f, 0.f));
				break;
			case 4:
				matrixRot = glm::rotate(glm::mat4(1.f), float(M_PI * 0.5f),
			...
			glm::mat4 matrixMove = glm::translate(glm::mat4(1.f),
					glm::vec3(0.0f, 0.0f, 1.0f));
//          glm::mat4 matrixMove = nv_math::translation_mat4(0.0f,0.0f,1.0f);

			Plane<TVertex>::add(geo, mat * matrixRot * matrixMove,
					configs[side][0], configs[side][1]);
		}
	}

	Box(int segments = 1)
	{
		add(*this, glm::mat4(1), segments, segments, segments);
	}
};
