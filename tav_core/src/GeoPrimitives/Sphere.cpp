...
namespace tav
{
Sphere::Sphere(float _radius, int _nrSlices, bool _cclockw, bool _triangulate,
		bool _genTexCoord) :
		GeoPrimitive(), radius(_radius), cclockw(_cclockw), numberSlices(
				_nrSlices), triangulate(_triangulate), genTexCoord(_genTexCoord)
{
	instAttribs = nullptr;
	maxNrInstances = 1;
	init();
...
void Sphere::init()
{
	...
	if (numberSlices < 3 || numberVertices > MAX_VERTICES
			|| numberIndices > MAX_INDICES)
	...
	for (i = 0; i < numberParallels + 1; i++)
	{
		for (j = 0; j < numberSlices + 1; j++)
		{
			...
			vertices[vertexIndex + 0] = radius * sinf(angleStep * (GLfloat) i)
					* sinf(angleStep * (GLfloat) j);
			vertices[vertexIndex + 1] = radius * cosf(angleStep * (GLfloat) i);
			vertices[vertexIndex + 2] = radius * sinf(angleStep * (GLfloat) i)
			...
			texCoords[texCoordsIndex + 0] = (GLfloat) j
					/ (GLfloat) numberSlices;
			texCoords[texCoordsIndex + 1] = 1.0f
					- (GLfloat) i / (GLfloat) numberParallels;

			// use quaternion to get the tangent vector
			glm::rotate(helpQuaternion,
					glm::vec4(texCoords[texCoordsIndex + 0])
							* GLfloat(M_PI * 2.0));
	...
	if (cclockw)
	{
		for (i = 0; i < numberParallels; i++)
		{
			for (j = 0; j < numberSlices; j++)
			{
				...
				indices[indexIndices++] = (i + 1) * (numberSlices + 1)
						+ (j + 1);
	...
	if (genTexCoord)
		...
		format = "position:3f,normal:3f,color:4f";

	vao = new VAO(format, usage, instAttribs, maxNrInstances, true);
	...
	if (triangulate)
	{
		...
		for (unsigned int j = 0; j < numberIndices; j++)
		{
			...
			for (unsigned int k = 0; k < 2; k++)
				tTexCoords[j * 2 + k] = texCoords[indices[j] * 2 + k];

			for (unsigned int k = 0; k < 4; k++)
				tColors[j * 4 + k] = colors[indices[j] * 4 + k];
		}

		vao->upload(POSITION, &tVertices[0], numberVertices);
		vao->upload(NORMAL, &tNormals[0], numberVertices);
		if (genTexCoord)
			vao->upload(TEXCOORD, &tTexCoords[0], numberVertices);
		vao->upload(COLOR, &tColors[0], numberVertices);

	}
	else
	{
		vao->upload(POSITION, &vertices[0], numberVertices);
		vao->upload(NORMAL, &normals[0], numberVertices);
		if (genTexCoord)
			vao->upload(TEXCOORD, &texCoords[0], numberVertices);
		vao->upload(COLOR, &colors[0], numberVertices);

		vao->setElemIndices(numberIndices, &indices[0]);
	}
}
...
{
	if (!triangulate)
		vao->drawElements(GL_TRIANGLES, _tfo, GL_TRIANGLES);
	else
		vao->draw(GL_TRIANGLES, _tfo, GL_TRIANGLES);
}
#else
void Sphere::draw()
{
...
unsigned int Sphere::getNrVertices()
{
