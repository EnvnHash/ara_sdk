 ...
 *
 *  possible effects: "blur",  "colorThreshold", "copyto",  "feedback", "flipH", "flipV", "inv",
 *  "keyStone", "mask", "medianBlur", "morpho", "raw", "resize", "subtr", "thres", "thres2", "undistort",
 *  "denoising"
 */
...
CvFxChain::CvFxChain(std::vector<std::string>* _args, int _width, int _height,
		int _nrChannels, int _bits, int _nrThreads) :
		height(_height), width(_width), nrChannels(_nrChannels), bits(_bits), nrThreads(
				_nrThreads), args(_args)
{
			...
			it != args->end(); ++it)
	{
		...
		{
			(*fxIt)->fxMat = new cv::Mat(height, width,
					CV_MAKETYPE(CV_8U, nrChannels));
		}
		else
		{
			(*fxIt)->fxMat = frame;
		...
		(*fxIt)->auxMat = NULL;
		// if ( std::strncmp( routine, "subtr", maxStringSize ) == 0 ) (*fxIt)->auxMat = &refpic_bw;
		if (std::strncmp(routine, "mask", maxStringSize) == 0)
			(*fxIt)->auxMat = &cam_mask_bw;
		...
		{
			(*fxIt)->auxMat = new cv::Mat(height, width,
					CV_MAKETYPE(CV_8U, nrChannels));
			(*fxIt)->cr = &colPointers;
		}
		...
		(*fxIt)->upSampWidth = static_cast<int>(static_cast<float>(width)
				* upSample.width);
		(*fxIt)->upSampHeight = static_cast<int>(static_cast<float>(height)
				* upSample.height);
		(*fxIt)->upSamp = cvSize((*fxIt)->upSampWidth, (*fxIt)->upSampHeight);
...
void CvFxChain::doCvRendering()
{
	...
	// process fxChain
	mtx.lock();
	for (std::vector<FxOpts*>::iterator it = fxChain->begin();
			it != fxChain->end(); ++it)
	{
		...
		if (std::strcmp((*it)->fxName, "keyStone") == 0)
		{
			...
			cv::warpPerspective(*result, *result, transmtx,
					cv::Size(width, height));
		}
		else
		{
			for (int i = 0; i < nrThreads; i++)
				startSubThread(i, result, (*it));
			for (int i = 0; i < nrThreads; i++)
			{
				sub_Thread[i]->join();
				delete sub_Thread[i];
...
void CvFxChain::imgProcess(int N, cv::Mat* inRoi, cv::Mat* outRoi,
		cv::Mat* auxMat, FxOpts* inOpts)
{
	...
	{
		cv::Canny(*inRoi, *outRoi, inOpts->cannyThresh, inOpts->cannyThresh * 2,
				inOpts->cannyKernelSize);

	}
	else if (std::strncmp(routine, "colorThreshold", maxStringSize) == 0)
	{
		cv::cvtColor(*inRoi, *auxMat, CV_RGB2HSV);
		cv::inRange(*auxMat,
				cv::Scalar(inOpts->cr->colRange[0][0],
						inOpts->cr->colRange[1][0], inOpts->cr->colRange[2][0]),
				cv::Scalar(inOpts->cr->colRange[0][1],
						inOpts->cr->colRange[1][1], inOpts->cr->colRange[2][1]),
				*outRoi);
	}
	...
	{
		cv::addWeighted(*inRoi, inOpts->fbkSrc, *auxMat, inOpts->fbkDst, 0.0f,
				*outRoi);
	...
	else if (std::strncmp(routine, "keyStone", maxStringSize) == 0)
	{
		...
		{
			thisKeyStoneSrc.push_back(
					cv::Point2f(
							std::min(
									std::max(keyStoneSrc[in].x,
											(float) inRois[N]->x),
									(float) (inRois[N]->x + inRois[N]->width)),
							std::min(
									std::max(keyStoneSrc[in].y,
											(float) inRois[N]->y),
									(float) (inRois[N]->y + inRois[N]->height))));
			thisKeyStoneDst.push_back(
					cv::Point2f(
							std::min(
									std::max(keyStoneDst[in].x,
											(float) inRois[N]->x),
									(float) (inRois[N]->x + inRois[N]->width)),
							std::min(
									std::max(keyStoneDst[in].y,
											(float) inRois[N]->y),
									(float) (inRois[N]->y + inRois[N]->height))));

		}
		inOpts->transmtx[N] = cv::getPerspectiveTransform(thisKeyStoneSrc,
				thisKeyStoneDst);
		cv::warpPerspective(*inRoi, *outRoi, inOpts->transmtx[N],
				inRoi->size());
	...
	else if (std::strncmp(routine, "morpho", maxStringSize) == 0)
	{
		int morph_elem = 1;
		int morph_size = 9;
		cv::Mat melement = cv::getStructuringElement(morph_elem,
				cv::Size(2 * morph_size + 1, 2 * morph_size + 1),
				cv::Point(morph_size, morph_size));
		// cv::morphologyEx(*inRoi, *outRoi, cv::MORPH_CLOSE, melement, cv::Point(-1,-1), 8); // bei hohen werten weniger cpu und gröbere kanten
	...
	{
		if (auxMat != 0)
			cv::subtract(*inRoi, *auxMat, *outRoi);

	}
	else if (std::strncmp(routine, "thres", maxStringSize) == 0)
	{
		cv::inRange(*inRoi, cv::Scalar(*inOpts->thres1), cv::Scalar(255),
				*outRoi);
//		cv::inRange(*inRoi,
//					cv::Scalar(*inOpts->thres1, *inOpts->thres1, *inOpts->thres1),
...
//					);
	...
	{
		cv::inRange(*inRoi, cv::Scalar(maxStringSize), cv::Scalar(255),
				*outRoi);

	}
	else if (std::strncmp(routine, "undistort", maxStringSize) == 0)
	{
		cv::undistort(*inRoi, *outRoi, *inOpts->cameraMatrix,
				*inOpts->distCoeffs, *inOpts->cameraMatrix);
	}
}
...
void CvFxChain::startSubThread(int N, cv::Mat* inMat, FxOpts* inOpts)
{
	...
	for (int y = 0; y < inOpts->mtRows; y++)
	{
		...
		{
			inRois[count] = new cv::Rect(inMat->cols / inOpts->mtCols * x,
					inMat->rows / inOpts->mtRows * y,
					inMat->cols / inOpts->mtCols, inMat->rows / inOpts->mtRows);

			if (std::strncmp(inOpts->fxName, "flip", maxStringSize) != 0)
			{
				outRois[count] = new cv::Rect(
						inOpts->fxMat->cols / inOpts->mtCols * x,
						inOpts->fxMat->rows / inOpts->mtRows * y,
						inOpts->fxMat->cols / inOpts->mtCols,
	...
	*subOutPics[N] = cv::Mat(*inOpts->fxMat, *outRois[N]);
	if (inOpts->auxMat != NULL
			&& (std::strncmp(inOpts->fxName, "morpho", maxStringSize) != 0))
		*subAuxPics[N] = cv::Mat(*inOpts->auxMat, *inRois[N]);

	sub_Thread[N] = new std::thread(&CvFxChain::imgProcess, this, N,
			subPics[N], subOutPics[N], subAuxPics[N], inOpts);
}
...
void CvFxChain::onKey(int key, int scancode, int action, int mods)
{
	if (action == GLFW_PRESS)
	{
		...
		case GLFW_KEY_S:
			...
			refpic = cv::imread("/projekte/programmierung/tav/data/refpic.jpg");
			cv::cvtColor(refpic, refpic_bw, CV_BGR2GRAY);
			break;
...
std::vector<FxOpts*>* CvFxChain::getFxChain()
{
	return fxChain;
}

int CvFxChain::getWidth()
{
	return static_cast<int>(width);
}

int CvFxChain::getHeight()
{
	return static_cast<int>(height);
}

uint8_t* CvFxChain::getImg()
{
	return fxChain->at(dispSrc)->fxMat->data;
}

cv::Mat* CvFxChain::getFinalImg()
{
	return result;
}

uint8_t* CvFxChain::getFinalImgData()
{
	return result->data;
}
...
void CvFxChain::setFdbkAmtSrc(double _fdbkAmt)
{
	...
	for (std::vector<std::string>::iterator it = args->begin();
			it != args->end(); ++it)
	{
		if (std::strcmp((*it).c_str(), "feedback") == 0)
			fxChain->at(ind)->fbkSrc = _fdbkAmt;
		ind++;
	}
}
...
void CvFxChain::setFdbkAmtDst(double _fdbkAmt)
{
	...
	{
		if (std::strcmp((*it).c_str(), "feedback") == 0)
			fxChain->at(ind)->fbkDst = _fdbkAmt;
		ind++;
	}
}
...
void CvFxChain::setThresh1(int _val)
{
	thres1 = _val;
}
...
void CvFxChain::setSubtrMat(cv::Mat* _subtrMat)
{
	for (std::vector<FxOpts*>::iterator it = fxChain->begin();
			it != fxChain->end(); ++it)
		if (std::strcmp((*it)->fxName, "subtr") == 0)
			(*it)->auxMat = _subtrMat;
}
