...
public:
	...
	CvFxChain(std::vector<std::string>* args, int _width, int _height,
			int _nrChannels, int _bits = 8, int _nrThreads = 4);
	~CvFxChain();
	uint8_t* procImg(uint8_t* img);
	void startSubThread(int N, cv::Mat* inMat, FxOpts* inOpts);
	void imgProcess(int N, cv::Mat* inRoi, cv::Mat* outRoi, cv::Mat* auxMat,
			FxOpts* inOpts);
	void doCvRendering();
