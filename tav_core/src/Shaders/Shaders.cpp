...
Shaders::Shaders(const char* comp_str, bool readFromFile)
{
	if (readFromFile){
		if (comp_str) comp = textFileRead(comp_str);
	} else {
		if (comp_str) comp = (char*) comp_str;
	}
...
Shaders::Shaders(const char* vert, const char* geom, const char* frag,
		bool readFromFile)
{
	if (readFromFile)
	{
...
Shaders::Shaders(const char* vert, const char* tessCtr, const char* tessEv,
		const char* geom, const char* frag, bool readFromFile)
{
	if (readFromFile)
	{
		vs = textFileRead(vert);
		if (tessCtr)
			cs = textFileRead(tessCtr);
		if (tessEv)
			es = textFileRead(tessEv);
		if (geom)
			gs = textFileRead(geom);
		fs = textFileRead(frag);
	}
...
GLuint Shaders::create()
{
	...
	if (program)
	{
		...
		if (comp){
			//std::cout << "Shaders::create setting program separable to true" << std::endl;
			glProgramParameteri(program, GL_PROGRAM_SEPARABLE, GL_TRUE);
			attachShader(program, GL_COMPUTE_SHADER, comp);
		}
		if (cs) attachShader(program, GL_TESS_CONTROL_SHADER, cs);
		if (es) attachShader(program, GL_TESS_EVALUATION_SHADER, es);
...
void Shaders::checkStatusLink(GLuint obj)
{
	...
	if (status != GL_TRUE)
	{
		std::cerr << "Shaders::checkStatusLink ERROR: shader didn´t link!!!" << std::endl;
		...
		std::cerr << "Shaders::checkStatusLink shader log: " << log << std::endl;
		std::cerr << "Shaders::checkStatusLink shader vertex code: log: " << vs << std::endl;

		if (gs != 0) std::cout << "Shaders::checkStatusLink shader geometry code: log: " << gs << std::endl;
		std::cerr << "Shaders::checkStatusLink shader fragment code: " << std::endl << fs << std::endl;
	}
}

//--------------------------------------------------------------

bool Shaders::checkStatusCompile(GLuint obj)
{
	GLint status = GL_FALSE, len = 10;
	...
	if (status != GL_TRUE)
	{
		std::cerr << "Shaders::checkStatusCompile ERROR: shader didn´t compile!!!" << std::endl;
		if ( glIsShader(obj) ) glGetShaderiv(obj, GL_INFO_LOG_LENGTH, &len);
		if ( glIsProgram(obj) ) glGetProgramiv(obj, GL_INFO_LOG_LENGTH, &len);
		...
		std::cout << "Shaders::checkStatusLink shader log: " << log << std::endl << std::endl;
		std::cout << "Shaders::checkStatusLink shader vertex code: " << std::endl << vs << std::endl;
		std::cout << "Shaders::checkStatusLink shader fragment code: " << std::endl << fs << std::endl;
...
void Shaders::attachShader(GLuint program, GLenum type, const char* src)
{
	if (debug)
	{
		...
		case GL_COMPUTE_SHADER:
			std::cout << "GL_COMPUTE_SHADER" << std::endl;
			break;
	...
	if (shader)
	{
		...
		std::cerr << "Shaders::attachShader Error, glCreateShader() failed" << std::endl;
	}
}
...
void Shaders::getGlVersions()
{
	...
	const char *slVerstr = (const char*) glGetString(
...
//--------------------------------------------------------------
void Shaders::setUniform2ui(const std::string & name, unsigned int v1,
		unsigned int v2)
{
	if (bLoaded)
	{
...
//--------------------------------------------------------------
void Shaders::setUniform3ui(const std::string & name, unsigned int v1,
...
//--------------------------------------------------------------
void Shaders::setUniform4ui(const std::string & name, unsigned int v1,
...
//--------------------------------------------------------------
void Shaders::setUniform4i(const std::string & name, int v1, int v2, int v3,
		int v4)
{
	if (bLoaded)
	{
...
//--------------------------------------------------------------
void Shaders::setUniform3f(const std::string & name, float v1, float v2,
		float v3)
{
	if (bLoaded)
	{
...
//--------------------------------------------------------------
void Shaders::setUniform4f(const std::string & name, float v1, float v2,
		float v3, float v4)
{
	if (bLoaded)
	{
...
void Shaders::setUniformMatrix3fv(const std::string & name, float* v, int count)
{
	if (bLoaded)
	{
		int loc = getUniformLocation(name);
		if (loc != -1)
			glUniformMatrix3fv(loc, count, GL_FALSE, v);
	}
}
