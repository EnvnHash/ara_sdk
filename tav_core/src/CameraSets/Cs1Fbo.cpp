...
Cs1Fbo::Cs1Fbo(sceneData* _scd, OSCData* _osc, std::vector<fboView*>* _fboViews, GWindowManager* _winMan) :
		CameraSet(1, _scd, _osc), winMan(_winMan)
{
	...
	cam[0] = new GLMCamera(GLMCamera::FRUSTUM,
			scrWidth, scrHeight,
			-1.f, 1.0f, -1.0f, 1.0f,                // left, right, bottom, top
			0.f, 0.f, 1.f,                           // camPos
...
void Cs1Fbo::initProto(ShaderProto* _proto)
{
...
void Cs1Fbo::render(SceneNode* _scene, double time, double dt, unsigned int ctxNr)
{
...
void Cs1Fbo::renderFbos(double time, double dt, unsigned int ctxNr)
{
...
//        fboDedist->drawTestPic();
	...
	if(reqSnapshot)
	{
		std::cout << "snapshot requested!!! " << fboSize.x <<  ", " << fboSize.y << std::endl;
		glDisable(GL_DEPTH_TEST);
		glDisable(GL_CULL_FACE);

		 cv::Mat bigpic (fboSize.y, fboSize.x, CV_8UC4);
...
void Cs1Fbo::preDisp(double time, double dt)
{
}

//--------------------------------------------------------------------------------

void Cs1Fbo::postRender()
{
}

//--------------------------------------------------------------------------------

void Cs1Fbo::onKey(int key, int scancode, int action, int mods)
{
...
void Cs1Fbo::mouseBut(GLFWwindow* window, int button, int action, int mods)
{
...
void Cs1Fbo::mouseCursor(GLFWwindow* window, double xpos, double ypos)
{
