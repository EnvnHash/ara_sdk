...
#include "pch.h"
#include "SPDirLightNoTex.h"

namespace tav
{
SPDirLightNoTex::SPDirLightNoTex(spData* _sData, ShaderCollector* _shCol) :
		ShaderProto(_sData, _shCol)
{
	...
	lightProp->setShininess(60.0f);
	lightProp->setStrength(0.5f);
}

SPDirLightNoTex::~SPDirLightNoTex()
{
}

//--------------------------------------------------------------------------------

void SPDirLightNoTex::defineVert(bool _multiCam)
{
	a.setVertStdInput();
	...
	std::string code = "//SPDirLightNoTex Directional Light Prototype\n"
			"out vec4 $O_Color;\n"
			"out vec3 $O_Normal;\n"
			"out vec4 $O_TexCoord;\n";
...
void SPDirLightNoTex::defineGeom(bool _multiCam)
{
}

//--------------------------------------------------------------------------------

void SPDirLightNoTex::defineFrag(bool _multiCam)
{
	a.setWorkShader(FRAGMENT);
	a.addCode("//SPDirLightNoTex Directional Light Prototype\n"
			...
			"vec3 rgb = min($I_Color.rgb * scatteredLight + reflectedLight, vec3(1.0));\n"
...
void SPDirLightNoTex::preRender(SceneNode* _scene, double time, double dt)
{
}

//--------------------------------------------------------------------------------

void SPDirLightNoTex::sendPar(int nrCams, camPar& cp, OSCData* osc, double time)
{
