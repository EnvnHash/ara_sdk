...
void UI_ElementsOverview::addSlider(UINode* root, int heightStep) {
    auto& slider = root->push<Slider>(0, heightStep * 3, 300, 30);
    slider.setValue(0.f);
}
...
void UI_ElementsOverview::addImage(UINode* root, int heightStep) {
    auto& image = root->push<Image>(0, heightStep * 16, 300, 200);
    image.setImg("test/test-tex.png", 4);
}

void UI_ElementsOverview::addImageButton(UINode* root, int heightStep) {
    auto& imageButt = root->push<ImageButton>(0, heightStep * 8, 25, 25);
...
void UI_ElementsOverview::addSpinner(UINode* root, int heightStep) {
    auto& spinner = root->push<Spinner>(0, heightStep * 9, 80, 80);
    spinner.setImage("test/spinner-1.png", 2);
}

void UI_ElementsOverview::addEditor(UINode* root, int heightStep) {
    auto& edit = root->push<UIEdit>(0, heightStep * 12, 300, 30);
...
void UI_ElementsOverview::addInfoDialogButton(UINode* root, int heightStep) {
    auto& butt2 = root->push<Button>(0, heightStep * 14, 100, 30);
...
void UI_ElementsOverview::addTable(UINode* root) {
    auto& table = root->push(make_unique<UITable>(350.f, 0.f, 800.f, 300.f, 4, 3));
...
void UI_ElementsOverview::addTabView(UINode* root) {
    ...
    auto& tabView = root->push<TabView>(350, 400, 800, 300, fgCol, bgCol);
    tabView.setPadding(10.f);

    auto l1 = tabView->addTab<Label>("Tab 1");
    l1.setText("TAB 1");

    auto& l2 = tabView->addTab<Label>("Tab 2");
    l2.setText("TAB 2");

    auto& l3 = tabView->addTab<Label>("Tab 3");
    l3.setText("TAB 3");
}