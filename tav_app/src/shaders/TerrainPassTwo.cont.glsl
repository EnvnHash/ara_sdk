...
uniform uint u_maxTessellationLevel; // 4 -> teilt das patch in u_maxTessellationLevel ^2 quadrate
uniform int u_quadrantStep;         // 3
uniform vec4 u_positionTextureSpace; // statisch (0,0,0,1)
uniform float u_detailStepS;
uniform float u_detailStepT;

void main(void)
{
//	float tessellationStepS = pow(2.0, float(u_maxTessellationLevel)); // 16
//	float tessellationStepT = pow(2.0, float(u_maxTessellationLevel)); // 16
    
    //float tessellationStep = 1.0 / pow(2.0, float(u_maxTessellationLevel)); // 16
//    float tessellationStepS = 1.0 / u_detailStepS; // 16
//    float tessellationStepT = 1.0 / u_detailStepT; // 16
...
//	int relativeQuadrantT = int((gl_in[0].gl_Position.z - u_positionTextureSpace.z) / tessellationStep);
    int relativeQuadrantS = int(gl_in[0].gl_Position.x / u_detailStepT);
    int relativeQuadrantT = int(gl_in[0].gl_Position.z / u_detailStepS);
	...
	bool leftBorder = ((absRelativeQuadrantS + 1) % u_quadrantStep == 0)
                        && (absRelativeQuadrantS == chebyshevDistance)
                        && (relativeQuadrantS <= 0);
    
	bool rightBorder = ((absRelativeQuadrantS + 1) % u_quadrantStep == 0)
                        && (absRelativeQuadrantS == chebyshevDistance)
                        && (relativeQuadrantS >= 0);

	bool bottomBorder = ((absRelativeQuadrantT + 1) % u_quadrantStep == 0)
                        && (absRelativeQuadrantT == chebyshevDistance)
                        && (relativeQuadrantT <= 0);
    
	bool topBorder = ((absRelativeQuadrantT + 1) % u_quadrantStep == 0)
                        && (absRelativeQuadrantT == chebyshevDistance)
                        && (relativeQuadrantT >= 0);
	...
	// Depending on the border, change the decrease level. So we do have no artifacts when changing from a smaller to a larger patch and vice versa.
