...
TEST(UITest, ReloadGLContextImageTests) {
    appRestartGL([&](UIApplication& app){
        app.getMainWindow()->getRootNode()->addChild<Image>(0, 0, 300, 300, nullptr, nullptr)->setImg("checkerboard_small.png", 1);
    }, [&](UIApplication& app){
...
TEST(UITest, ReloadGLContextLabelTests) {
    appRestartGL([&](UIApplication& app){
        auto label = app.getMainWindow()->getRootNode()->addChild<Label>(10, 10, 400, 100);
