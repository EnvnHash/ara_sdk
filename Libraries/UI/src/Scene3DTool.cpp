...
void Scene3DTool::connectUINodes() {
    m_rightSide = (Div *)this->getNode("rightSide");
    if (!m_rightSide) LOGE << "ERROR in config.xml, Missing Entry with ID \"rightSide\"";

    m_sceneContainer = (Div *)this->getNode("container");
    if (!m_sceneContainer) {
    ...
    m_sceneBackImg = (Image *)this->getNode("backImg");
    if (!m_sceneBackImg) {
    ...
    if (!m_scene3D)
        ...
        m_scene3D->setDeselectAllCb([this] {
            if (m_toolBar && m_toolBar->isVisible()) objectToolClicked(ToolBarIcon::select);
        });
    ...
    m_axesGizmo = (Gizmo *)this->getNode("gizmo");
    if (!m_axesGizmo) {
    ...
    m_transWidget = (TransformWidget *)this->getNode("transWidget");
    if (!m_transWidget) {
    ...
    m_toolBar = (Gizmo *)this->getNode("toolBar");
    if (!m_toolBar) LOGE << "ERROR in config.xml, Missing Entry with ID \"toolBar\"";
    ...
    // menu icons
    for (auto &it : str_to_enum) {
        m_toolBarIcons[it.first] = (ImageButton *)this->getNode(it.second);
        if (!m_toolBarIcons[it.first])
            LOGE << "ERROR in config.xml, Missing Entry with ID \"" << it.second << "\"";
        else {
            m_toolBarIcons[it.first]->setFocusAllowed(false);
            m_toolBarIcons[it.first]->setLod(1.f);

            if (it.first != ToolBarIcon::coarseFine) m_toolBarIcons[it.first]->setIsToggle(false);

            if (it.first == ToolBarIcon::translate || it.first == ToolBarIcon::rotate)
                m_toolBarIcons[it.first]->setDisabled(true, true);

            if (it.first == ToolBarIcon::select || it.first == ToolBarIcon::translate ||
                it.first == ToolBarIcon::rotate || it.first == ToolBarIcon::twWidget) {
                m_objToolIcons[it.first] = (ImageButton *)this->getNode(it.second);

                if (it.first == ToolBarIcon::translate || it.first == ToolBarIcon::rotate ||
                    it.first == ToolBarIcon::twWidget)
                    m_objTransIcons[it.first] = (ImageButton *)this->getNode(it.second);
            } else if (it.first == ToolBarIcon::pan || it.first == ToolBarIcon::orbit || it.first == ToolBarIcon::zoom)
                m_viewToolIcons[it.first] = (ImageButton *)this->getNode(it.second);
        }
    }
...
void Scene3DTool::setKeyTransStep(float fine, float normal, float coarse) {
    if (m_scene3D) m_scene3D->setKeyTransStep(fine, normal, coarse);

    if (m_transWidget) m_transWidget->setKeyTransStep(fine, normal, coarse);
}

void Scene3DTool::setKeyRotStep(float fine, float normal, float coarse) {
    if (m_scene3D) m_scene3D->setKeyRotStep(glm::radians(fine), glm::radians(normal), glm::radians(coarse));

    if (m_transWidget) m_transWidget->setKeyRotStep(fine, normal, coarse);
}
