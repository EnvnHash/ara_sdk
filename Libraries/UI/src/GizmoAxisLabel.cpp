...
GizmoAxisLabel::GizmoAxisLabel(std::string file, int mipMapLevel, bool keep_aspect, align ax, valign ay)
    : Image(std::move(file), mipMapLevel, keep_aspect, ax, ay) {
    setName(getTypeName<GizmoAxisLabel>());
    m_drawImmediate = true;
}
...
void GizmoAxisLabel::pushVaoUpdtOffsets() {
    ...
    if (m_hasLabel && m_imgDB.drawSet) m_imgDB.drawSet->updtNodes.emplace_back(m_imgDB.getUpdtPair());
}
...
void GizmoAxisLabel::mouseOut(hidData* data) {
    m_setStyleFunc[state::none][styleInit::x] = [this] {};
    m_setStyleFunc[state::none][styleInit::y] = [this] {};

    UINode::mouseOut(data);  // call this to update styles

    // since this is rendered out of tree, manually call updateDrawData
    if (m_gizmo) m_gizmo->m_updtDrawData = true;
    ...
    if (m_gizmo && m_gizmo->getSharedRes()) m_gizmo->getSharedRes()->requestRedraw = true;
}

void GizmoAxisLabel::mouseDrag(hidData* data) {
    if (m_gizmo) m_gizmo->drag(data);
    if (data->hit) data->consumed = true;
}

void GizmoAxisLabel::mouseDown(hidData* data) {
    if (!m_gizmo || data->clickedObjId != m_objIdMin) return;
...
void GizmoAxisLabel::mouseUp(hidData* data) {
    if (!m_leftPressed || !m_gizmo || data->clickedObjId != m_objIdMin) return;

    if (m_resetExcludeFromStyles) {
        m_gizmo->excludeLabelsFromStyles(false);  // Note: must be called before UINode::mouseUp and
    ...
    if (!data->dragging) {
        ...
        // call
        if (data->releasedObjId != m_objIdMin && m_gizmo->m_lastMouseHoverData) setSelected(false, true);
    }

    m_mousePosRel = m_gizmo->getGizmoRelMousePos(data->mousePos);

    if (m_gizmo->getCamera()) m_gizmo->getCamera()->mouseUpLeft(m_mousePosRel.x, m_mousePosRel.y);
...
void GizmoAxisLabel::setGizmoParent(Gizmo* gizmo) {
    m_gizmo = gizmo;
    if (m_gizmo) setSharedRes(m_gizmo->getAuxSharedRes());
}
