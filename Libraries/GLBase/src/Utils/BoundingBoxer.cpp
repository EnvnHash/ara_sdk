...
BoundingBoxer::BoundingBoxer(GLBase *glbase) : m_shCol(glbase->shaderCollector()) {
    fbo = make_unique<FBO>(glbase, 6, 1, GL_R32F, GL_TEXTURE_2D, false, 1, 1, 1, GL_CLAMP_TO_EDGE, false);
...
void BoundingBoxer::begin() {
    ...
    /*
        m_glbase.stateMan().set(glFun::Disable, {GL_CULL_FACE});
        m_glbase.stateMan().set(glFun::Disable, {GL_DEPTH_TEST});
        m_glbase.stateMan().set(glFun::Enable, {GL_BLEND});
        m_glbase.stateMan().set(glFun::BlendEquation, {GL_MAX});
        m_glbase.stateMan().set(glFun::BlendFunc, {GL_ONE, GL_ONE});
    */
...
void BoundingBoxer::end() {
    ...
    // m_glbase.stateMan().set(glFun::BlendEquation, {GL_FUNC_ADD});
    // m_glbase.stateMan().set(glFun::BlendFunc, {GL_SRC_ALPHA,
    // GL_ONE_MINUS_SRC_ALPHA});
    glBlendEquation(GL_FUNC_ADD);
    glBlendFunc(GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);

    for (int i = 0; i < 3; i++) {
        if (-result[i * 2] != std::numeric_limits<float>::max()) boundMin[i] = -result[i * 2];

        if (-result[i * 2 + 1] != std::numeric_limits<float>::max()) boundMax[i] = result[i * 2 + 1];
    }

    for (int i = 0; i < 3; i++) center[i] = (-boundMin[i] + boundMax[i]) * 0.5f + boundMin[i];
}
