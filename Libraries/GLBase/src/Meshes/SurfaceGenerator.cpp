...
void SurfaceGenerator::init(surfaceType type, ShaderCollector *shCol) {
    ...
    if (!m_vao) m_vao = make_unique<VAO>("position:3f,normal:3f,texCoord:3f", GL_STATIC_DRAW);
    ...
    if (!m_genVertShdr) {
        ...
        m_genVertShdr    = shCol->add("SurfaceGenerator", m_vertShdrSrc.c_str());
        m_genElemIndShdr = shCol->add("SurfGenElemenInd", m_elemIndShdrSrc.c_str());
    }

    // calculate the grid resolution
    if (m_genVertShdr && ((type == FLAT && m_width > 0.f && m_height > 0.f) ||
                          ((type == CYLINDER || type == PANADOME || type == DOME) && m_radius > 1.f))) {
        float hCircum, vCircum;
        if (type == FLAT) {
            hCircum = m_width;
            vCircum = m_height;
        } else if (type == CYLINDER) {
            // calculate the horizontal circumference (in mm)
            hCircum = (-m_leftAng + m_rightAng) / 360.f * (2.f * (float)M_PI * m_radius);
            // calculate the vertical circumference
            vCircum = (float)M_PI * m_height;
        } else if (type == DOME) {
            hCircum = 2.f * (float)M_PI * m_radius;
            vCircum = 0.5f * (float)M_PI * m_radius;
        } else if (type == PANADOME) {
            hCircum = (-m_leftAng + m_rightAng) / 360.f * (2.f * (float)M_PI * m_radius);
            vCircum = (-m_bottAng + m_topAng) / 360.f * (2.f * (float)M_PI * m_radius);
        }

        m_qHoriVerts = (uint32_t)(m_meshVertsPerMM * hCircum);
        m_qVertVerts = (uint32_t)(m_meshVertsPerMM * vCircum);
...
void SurfaceGenerator::resize() {
    ...
    if (!m_pos) m_pos = make_unique<ShaderBuffer<custVec3>>(m_qVertices);
    if (!m_norm) m_norm = make_unique<ShaderBuffer<custVec3>>(m_qVertices);
    if (!m_texc) m_texc = make_unique<ShaderBuffer<custVec3>>(m_qVertices);

    // two triangles per segment = 6 vertices per MeshCell
    if (!m_ind) m_ind = make_unique<ShaderBuffer<uint32_t>>(m_qIndices);
    ...
    if (m_qIndices > m_ind->getSize()) m_ind->resize(m_qIndices);
}

void SurfaceGenerator::generateVert() {
    // run the compute shader, generate!!
    m_genVertShdr->begin();
    m_genVertShdr->bindProgramPipeline();

    m_genVertShdr->setUniform1ui("type", (uint32_t)m_type);
    ...
    if (m_type != FLAT) m_genVertShdr->setUniform1f("radius", m_radius * 0.001f);

    if (m_type == FLAT || m_type == CYLINDER) m_genVertShdr->setUniform1f("height", m_height * 0.001f);
    if (m_type == FLAT) m_genVertShdr->setUniform1f("width", m_width * 0.001f);
    ...
    // round up
    glDispatchCompute((GLuint)std::ceil((float)m_pos->getSize() / (float)m_work_group_size), 1, 1);
    ...
    m_genVertShdr->unbindProgramPipeline();
    m_genVertShdr->end();
}

void SurfaceGenerator::generateElemInd() {
    ...
    glDispatchCompute((GLuint)std::ceil((float)m_ind->getSize() / (float)m_work_group_size), 1, 1);
    glMemoryBarrier(GL_SHADER_STORAGE_BARRIER_BIT);
    glBindBufferBase(GL_SHADER_STORAGE_BUFFER, 1, 0);

    m_genElemIndShdr->unbindProgramPipeline();
    m_genElemIndShdr->end();
}

void SurfaceGenerator::exportCollada(std::string path, std::string formatId) {
#ifdef ARA_USE_ASSIMP
    if (m_qVertices > 0)
        m_exporter.singleMeshExport(m_qVertices, m_qIndices, m_pos.get(), m_norm.get(), m_texc.get(), m_ind.get(), path,
                                    formatId);
#endif
}

void SurfaceGenerator::setTypeName(std::string typeName) {
