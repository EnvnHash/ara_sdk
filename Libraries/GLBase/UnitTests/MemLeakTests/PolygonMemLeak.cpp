...
namespace ara::GLBaseUnitTest::PolygonMemLeak {
    ...
    TEST(GLBaseTest, PolygonMemLeak
    ) {
...
i<10; i++)
{
auto poly = make_unique<Polygon>(&shCol);
poly->

init();

auto p = poly->addPoint(0, -0.5f, 0.5f, 0.f, 1.f);
p->
texBasePoint = true;

p = poly->addPoint(0, -0.5f, -0.5f, 0.f, 0.f);
p->
texBasePoint = true;

p = poly->addPoint(0, 0.5f, -0.5f, 1.f, 0.f);
p->
texBasePoint = true;

p = poly->addPoint(0, 0.5f, 0.5f, 1.f, 1.f);
p->
texBasePoint = true;
