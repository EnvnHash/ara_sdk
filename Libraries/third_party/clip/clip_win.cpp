...
public:
  ...
  explicit Hglobal(HGLOBAL handle)
    : m_handle(handle) {
  }

  explicit Hglobal(size_t len)
    : m_handle(GlobalAlloc(GHND, len)) {
  }
  ...
  if (f == text_format()) {
    if (IsClipboardFormatAvailable(CF_UNICODETEXT)) {
      ...
      if (hglobal) {
        ...
        if (lpstr) {
          size_t reqsize =
            WideCharToMultiByte(CP_UTF8, 0, lpstr, -1, nullptr, 0, nullptr, nullptr);
...
bool lock::impl::set_image(const image &image) {
  ...
  Hglobal hmem(
    GlobalAlloc(
      GHND,
      sizeof(BITMAPV5HEADER) + palette_colors * sizeof(RGBQUAD) + out_spec.bytes_per_row * out_spec.height));
  if (!hmem)
    return false;
