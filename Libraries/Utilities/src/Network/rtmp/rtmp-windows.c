...
static bool socket_event(struct rtmp_stream *stream, bool *can_write, uint64_t last_send_time) {
    ...
    if (net_events.lNetworkEvents & FD_WRITE) *can_write = true;
    ...
    if (net_events.lNetworkEvents & FD_READ) {
        ...
        for (;;) {
            ...
            if (ret == -1) {
                ...
                if (err_code == WSAEWOULDBLOCK) break;
    ...
    if (ret == 0) {
        ...
        if (ret == 0) {
            if (cur_tcp_bufsize < (int)ideal_send_backlog) {
                ...
                setsockopt(stream->rtmp.m_sb.sb_socket, SOL_SOCKET, SO_SNDBUF, (const char *)&bufsize, sizeof(bufsize));
...
static enum data_ret write_data(struct rtmp_stream *stream, bool *can_write, uint64_t *last_send_time,
    ...
    if (stream->write_buf_len <= 1000) exit_loop = true;

    pthread_mutex_unlock(&stream->write_buf_mutex);

    if (delay_time) os_sleep_ms(delay_time);
...
static inline void socket_thread_windows_internal(struct rtmp_stream *stream) {
    ...
    if (!stream->disable_send_window_optimization) {
        ...
        blog(LOG_INFO,
             "socket_thread_windows: Send window "
    ...
    for (;;) {
        ...
        if (status == WAIT_OBJECT_0) {
            /* Socket event */
            if (!socket_event(stream, &can_write, last_send_time)) return;
        ...
        if (can_write) {
            for (;;) {
                enum data_ret ret = write_data(stream, &can_write, &last_send_time, latency_packet_size, delay_time);

                switch (ret) {
                    case RET_BREAK: goto exit_write_loop;
                    case RET_FATAL: return;
                    case RET_CONTINUE:;
                }
            }
        }
