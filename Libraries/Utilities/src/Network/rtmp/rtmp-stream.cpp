...
const char *rtmp_stream_getname(void *unused) {
    UNUSED_PARAMETER(unused);
    return "RTMPStream";
}

void log_rtmp(int level, const char *format, va_list args) {
    if (level > RTMP_LOGWARNING) return;
...
void free_packets(rtmp_stream *stream) {
    size_t num_packets;

    stream->packets_mutex.lock();

    num_packets = num_buffered_packets(stream);
    if (num_packets) LOG << "Freeing " << num_packets << " remaining packets";
...
void rtmp_stream_destroy(void *data) {
    rtmp_stream *stream = (rtmp_stream *)data;

    if (stopping(stream) && !connecting(stream)) {
        stream->send_thread.join();

    } else if (connecting(stream) || active(stream)) {
        if (stream->connecting) stream->connect_thread.join();

        stream->stop_ts = 0;
        stream->stop_event.notify();

        if (active(stream)) {
            stream->send_sem.notify();
...
rtmp_stream &&rtmp_stream_create(obs_data *settings, obs_output *output) {
    rtmp_stream stream;
...
void rtmp_stream_stop(void *data, uint64_t ts) {
    auto stream = (rtmp_stream *)data;

    if (stopping(stream) && ts != 0) return;

    if (connecting(stream)) stream->connect_thread.join();

    stream->stop_ts = ts / 1000ULL;

    if (ts) stream->shutdown_timeout_ts = ts + (uint64_t)stream->max_shutdown_time_sec * 1000000000ULL;

    if (active(stream)) {
        stream->stop_event.notify();

        if (stream->stop_ts == 0) stream->send_sem.notify();
    } else {
        obs_output_signal_stop(stream->output, 0);
    }
}
...
static inline bool get_next_packet(struct rtmp_stream *stream, struct
encoder_packet *packet)
{
...
static int socket_queue_data(RTMPSockBuf *sb, const char *data, int len, void
*arg)
{
    UNUSED_PARAMETER(sb);
...
static int send_packet(struct rtmp_stream *stream, struct encoder_packet
*packet, bool is_header, size_t idx)
{
    uint8_t *data;
    size_t size;
    int recv_size = 0;
...
static inline bool can_shutdown_stream(struct rtmp_stream *stream, struct
encoder_packet *packet)
{
    ...
    if (!msg) {
        switch (stream->rtmp.last_error_code) {
            case -0x2700:
                ...
                break;
            case -0x7680:
                msg = "Failed to load root certificates for a secure TLS
connection." #if defined(__linux__) " Check you have an up to date root
certificate bundle in /etc/ssl/certs." #endif
        ;
            break;
        }
    }
...
static bool send_audio_header(struct rtmp_stream *stream, size_t idx, bool
*next)
{
    ...
    struct encoder_packet packet = {.type = OBS_ENCODER_AUDIO, .timebase_den =
1};
        ...
        if (ioctlsocket(stream->rtmp.m_sb.sb_socket, FIONBIO, &one))
        {
            ...
            if (vencoder)
            {
                ...
                if (params)
                {
                    ...
                    if (!bitrate)
                    {
                        warn("Video encoder didn't return a valid bitrate, new
network code may function poorly. Low latency mode disabled.");
        ...
        {
            warn("Disconnected while attempting to send additional metadata");
            return OBS_OUTPUT_DISCONNECTED;
        }
...
static void win32_log_interface_type(struct rtmp_stream *stream)
{
    ...
    static int try_connect(struct rtmp_stream *stream)
        ...
        // otherwise the data sent/received will not parse correctly on the
other end RTMP_Reset(&stream->rtmp);
...
(float)drop_threshold;
        ...
        // alternatively, drop only pframes: (!pframes && stream->dbr_enabled)
        // but let's test without dropping frames  at all first
        if (stream->dbr_enabled) {
...
desired priority if (packet->drop_priority < stream->min_priority) {
