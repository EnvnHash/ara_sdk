...
void netif_log_saddrs(struct netif_saddr_data *sd) {
    for (size_t i = 0; i < sd->addrs.num; i++) info("\t\t%s", sd->addrs.array[i].name);
}
...
bool netif_str_to_addr(struct sockaddr_storage *out, int *addr_len, const char *addr) {
    ...
    *addr_len      = sizeof(*out);
    ...
    if (ret == SOCKET_ERROR) warn("Could not parse address, error code: %d", GetLastError());
    return ret != SOCKET_ERROR;
#else
    *addr_len = ipv6 ? sizeof(struct sockaddr_in6) : sizeof(struct sockaddr_in);
    void *dst = ipv6 ? &((struct sockaddr_in6 *)out)->sin6_addr : &((struct sockaddr_in *)out)->sin_addr;
    if (inet_pton(out->ss_family, addr, dst)) return true;
...
is_loopback(struct ifaddrs *ifa) {
...
static inline void netif_get_addrs_nix(struct netif_saddr_data *ifaddrs) {
    ...
    for (ifa = ifaddr; ifa != NULL; ifa = ifa->ifa_next) {
        if (ifa->ifa_addr == NULL || is_loopback(ifa)) continue;
...
static inline void netif_get_addrs_win32(struct netif_saddr_data *ifaddrs) {
    ...
    for (cur_adap = adapter; !!cur_adap; cur_adap = cur_adap->Next) {
        ...
        if (cur_adap->OperStatus != IfOperStatusUp || cur_adap->IfType == IF_TYPE_SOFTWARE_LOOPBACK) continue;
        ...
        for (; !!unicast; unicast = unicast->Next) {
            ...
            if (family == AF_INET || family == AF_INET6) netif_push(socket_addr.lpSockaddr, ifaddrs, adap_name);
        }
