...
static int loadShCore() {
    if (_hShcoreDLL) return FALSE;

    _hShcoreDLL = ::LoadLibraryA("shcore.dll");
    if (!_hShcoreDLL) return FALSE;

    _pFuncGetDpiForMonitor = (LPFUNC_GETDPIFORMONITOR)::GetProcAddress(_hShcoreDLL, "GetDpiForMonitor");
...
static int unloadShCore() {
    ...
    if (_hShcoreDLL) bRes = ::FreeLibrary(_hShcoreDLL);

    return bRes;
}

static HRESULT GetDpiForMonitor(HMONITOR hMon, int dpiType, UINT* dpiX, UINT* dpiY) {
    if (!_pFuncGetDpiForMonitor) loadShCore();

    if (_pFuncGetDpiForMonitor) return (_pFuncGetDpiForMonitor)(hMon, dpiType, dpiX, dpiY);
...
static int getDisplayScale(HWND hwnd, float* xscale, float* yscale) {
    ...
    if (xscale) *xscale = xdpi / 96.f;
    if (yscale) *yscale = ydpi / 96.f;
#endif
    return 1;
}
