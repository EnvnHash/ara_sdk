...
SNGridFloor::SNGridFloor(sceneData* sd) : SceneNode(sd) {
    ...
    m_protoName[renderPass::scene] = getTypeName<SPGridFloor>();  // force the use of the GridFloor ShaderPrototype if it is present

    SceneNode::scale({20.0f, 20.0f, 1.0f});
}

void SNGridFloor::update(double time, double dt, CameraSet* cs) {
    if (!m_rotationSet) {
        switch (m_basePlane) {
            case basePlane::xz: SceneNode::rotate(glm::radians(90.f), {1.f, 0.f, 0.f}); break;
            case basePlane::xy: SceneNode::rotate(glm::radians(0.f), {1.f, 0.f, 0.f}); break;
            case basePlane::yz: SceneNode::rotate(glm::radians(90.f), {0.f, 1.f, 0.f}); break;
        }
...
void SNGridFloor::draw(double time, double dt, CameraSet* cs, Shaders* shader, renderPass pass, TFO* tfo) {
    if (pass == renderPass::objectMap || pass == renderPass::scene) {
