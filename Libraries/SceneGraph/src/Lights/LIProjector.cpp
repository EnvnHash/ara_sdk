...
void LIProjector::setup(bool force) {
    ...
    s_lightProp.setConeDirection(s_direction.x, s_direction.y, s_direction.z);  // must point opposite of eye dir
    s_lightProp.setDirection(s_direction.x, s_direction.y, s_direction.z);   // by scaling this, the intensity varies
    ...
    auto lightCenter = m_transVec + s_direction;

    // rotate up vector
    auto upVec = normalize(vec3(m_rotMat * vec4(0.f, 1.f, 0.f, 0.f)));
    s_view_mat = lookAt(m_transVec, lightCenter, upVec);
