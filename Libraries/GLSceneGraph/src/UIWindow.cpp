...
UIWindow::UIWindow(GLBase *glbase, int width, int height, int shiftX, int shiftY, bool osDecoration, bool transparentFB,
    ...
    if (!initToCurrentCtx) {
        ...
        // set hid callbacks, must be done this way, as GlFW expects static
        // c-style callbacks, which is not possible when aiming modular c++
        // style window architectures
        ...
        std::unordered_map<void *, Quad *> test;
    ...
    if (!s_inited) {
        ...
        // define the stdQuad (a VAO Collector is used, in order to avoid to
        // optimize memory resources
        m_quad = make_unique<Quad>(0.f, 0.f, 1.f, 1.f, vec3(0.f, 0.f, 1.f), 1.f, 0.f, 0.f, 1.f, nullptr, 1, false);
        m_normQuad =
            make_unique<Quad>(-1.f, -1.f, 2.f, 2.f, vec3(0.f, 0.f, 1.f), 1.f, 0.f, 0.f, 1.f, nullptr, 1, false);
        ...
        if (!initToCurrentCtx) {
            ...
            // since the GLFWWindow is not part of UIWindow, these functions
            // have to be bound this way
            m_menuBar->setCloseFunc([this] {
                // we are in gl HID thread of this UIWindow
                if (m_closeFunc) {
                    m_closeFunc(this);
