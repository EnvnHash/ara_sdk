...
VideoSource::VideoSource(const std::string sFileName, FrameQueue *pFrameQueue, bool _loop):
hVideoSource_(0)
{
    ...
    oSourceData_.videoEndCb = 0;
    oSourceData_.loop = _loop;
...
void VideoSource::ReloadVideo(const std::string sFileName, FrameQueue *pFrameQueue,
		VideoParser *pVideoParser, bool _loop)
{
	...
	if (oResult == CUDA_ERROR_FILE_NOT_FOUND){
		printf("VideoSource::ReloadVideo Error file not found, file: %s \n", sFileName.c_str());
	}
...
CUVIDEOFORMAT VideoSource::format() const
{
...
void VideoSource::start()
{
    ...
    if (oResult == CUDA_ERROR_INVALID_HANDLE)
        printf("VideoSource::start Error: CUDA_ERROR_INVALID_HANDLE\n");
...
int VideoSource::HandleVideoData(void *pUserData, CUVIDSOURCEDATAPACKET *pPacket)
{
    ...
    {
        if(!pVideoSourceData->loop)
        	pVideoSourceData->pFrameQueue->endDecode();

        if((pPacket->flags & CUVID_PKT_ENDOFSTREAM) && pVideoSourceData->videoEndCb)
       		pVideoSourceData->videoEndCb();
    }
