...
MediaRecorder::MediaRecorder(const char* _fname, PAudio* _pa, int _inWidth, int _inHeight,
		...
		pa(_pa), fileName(_fname), bufferReadPtr(-1), inWidth(_inWidth), inHeight(_inHeight), monRefRate(_monRefRate),
		nrBufferFrames(40),  forceCodec(_forceCodec),
		num_pbos(4), glDownloadFmt(GL_BGR), flipH(true)
#else
MediaRecorder::MediaRecorder(const char* _fname, int _inWidth, int _inHeight,
		int _monRefRate, const char* _forceCodec):
		fileName(_fname), bufferReadPtr(-1), inWidth(_inWidth), inHeight(_inHeight), monRefRate(_monRefRate),
		nrBufferFrames(40),  forceCodec(_forceCodec),
		num_pbos(4), glDownloadFmt(GL_BGR), flipH(true)
#endif
{
	...
	for (unsigned int i=0; i<nrBufferFrames; i++)
...
//    if (audio_st) open_audio(oc, audio_codec, audio_st);
	...
	unsigned int nrMixChans = static_cast<unsigned int>( (float(pa->getMaxNrInChannels()) / float(n_audio_st.st->codecpar->channels)) + 0.5f);

	// make mixDownMap
	for (int chan=0; chan<n_audio_st.st->codecpar->channels; chan++)
		for (unsigned int i=0;i<nrMixChans;i++)
			if ( chan + i * nrMixChans < pa->getMaxNrInChannels() )
	...
	pa->setExtCallback(std::bind(&MediaRecorder::mediaRecAudioDataCallback, this, std::placeholders::_1));
...
*/
    ...
    m_Thread = new std::thread(&MediaRecorder::recThread, this);
}
...
//void MediaRecorder::mediaRecAudioDataCallback(unsigned int numChans, int frames, int offset, int codecFrameSize,
//		unsigned int codecNrChans, std::vector< std::vector<unsigned int> >* mixDownMap, float** sampData,
//		std::vector<float>* inSampQ)

#ifdef WITH_AUDIO

void MediaRecorder::mediaRecAudioDataCallback(PAudio::paPostProcData* paData)
{
	float mixSamp = 0.f;
	...
	} else
	{
		...
		for (unsigned int samp=0; samp<paData->frameCount; samp++)
			...
			{
				mixSamp = 0.f;
				for ( unsigned int i=0;i< paData->mixDownMap->at(chan).size();i++)
					mixSamp += paData->sampData[ paData->mixDownMap->at(chan)[i] ][paData->offset + samp];
