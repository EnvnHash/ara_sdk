...
namespace tav
{
SNSingleTextBlock::SNSingleTextBlock(sceneData* _scd,
		...
		SceneNode(_scd, _sceneArgs), text("")
{
	...
	for (int i = 0; i < 2; i++)
	{
		...
		{
			std::cout << "assign _Cols[" << i << "]" << " scd->colors["
					<< static_cast<unsigned int>(sceneArgs->at(key)) << "]"
					<< std::endl;
			...
			std::cout << "cols[" << i << "]" << glm::to_string(cols[i])
					<< std::endl;
	...
	if (_sceneArgs->find("font0") != _sceneArgs->end())
		textBlock = new NVTextBlock(shCol,
				fonts[static_cast<int>(_sceneArgs->at("font0"))], 20.f, 4);
	else
		std::cout << "SNSingleTextBlock font0 definition not found in setup xml"
				<< std::endl;
...
void SNSingleTextBlock::draw(double time, double dt, camPar* cp,
