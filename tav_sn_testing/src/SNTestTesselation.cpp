...
#include "SNTestTesselation.h"

namespace tav
{
    SNTestTesselation::SNTestTesselation(sceneData* _scd, std::map<std::string, float>* _sceneArgs) : SceneNode(_scd, _sceneArgs)
    {
    ...
    SNTestTesselation::~SNTestTesselation()
    {
    	delete part;
    	delete colShader;
    }
    ...
    void SNTestTesselation::draw(double time, double dt, camPar* cp, Shaders* _shader, TFO* _tfo)
    {
        if (_tfo) {
    ...
    void SNTestTesselation::update(double time, double dt)
    {}
}
