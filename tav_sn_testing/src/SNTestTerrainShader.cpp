...
#include "SNTestTerrainShader.h"

#define STRINGIFY(A) #A

namespace tav
{
    SNTestTerrainShader::SNTestTerrainShader(sceneData* _scd, std::map<std::string, float>* _sceneArgs) :
    ...
    SNTestTerrainShader::~SNTestTerrainShader()
    {
        delete quad;
    }
    
    
    void SNTestTerrainShader::draw(double time, double dt, camPar* cp, Shaders* _shader, TFO* _tfo)
    {
        if (_tfo) {
...
//        sendStdShaderInit(_shader);
        ...
        shdr->setUniform1f("iGlobalTime", time);
        shdr->setUniform4f("iMouse", 0.0, 0.0, 0.0, 0.0);
        shdr->setUniform3f("iResolution", float(scd->screenWidth), float(scd->screenHeight), 1.0);
        
        quad->draw(_tfo);
    }
    
    
    void SNTestTerrainShader::buildTerShader()
    {
        shdr = shCol->addCheckShader("terrainRendering", "shaders/terrain_mountain.vert", "shaders/terrain_mountain.frag");
...
#define MOD3 vec3(3.07965, 7.1235, 4.998784)
        ...
        frag = "// Mountain terrain shader, frag\n"+shdr_Header+frag;
        
        shdr = shCol->addCheckShaderText("terrainMountain", vert.c_str(), frag.c_str());
         */
    }
    
    
    void SNTestTerrainShader::update(double time, double dt)
    {}
